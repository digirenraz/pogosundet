...
- [ ] **Seed the gyms table** — a ready-made script with **152 Frederikssund-municipality gyms** (names + exact coordinates, from collect.dk with the operators' permission) is at [`supabase/seeds/001_gyms_frederikssund_collect_dk.sql`](../supabase/seeds/001_gyms_frederikssund_collect_dk.sql): copy → paste into the Supabase SQL editor → Run (re-runnable, never overwrites). Then spot-check on prod: open the raid form and type e.g. "Frede" — the autocomplete should suggest real gyms. Still **non-blocking** for everything except the two coordinate-dependent follow-ups (nearby-gym suggestions, "show in maps"), which this unblocks.
