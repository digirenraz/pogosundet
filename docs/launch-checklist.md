...
- [ ] **Apply migration 018 (gyms) BEFORE merging `slice/gyms-database`** (added 2026-06-10, issue #93). Run `supabase/migrations/018_gyms.sql` in the SQL editor — the gym autocomplete queries the `gyms` table as soon as the code deploys, and preview/prod share one Supabase project (same apply-before-deploy ordering as 015/017; the form degrades to free-text if the table is missing, but don't rely on that). Then seed gyms per [`docs/gyms-seeding.md`](gyms-seeding.md) — **incremental and non-blocking**: the autocomplete works with whatever rows exist, and auto-learn adds name-only rows from day one.
