...
- [ ] **Redeploy all four push Edge Functions for the caller-auth fix** (added 2026-06-12, `fix/edge-function-caller-auth`; security review Finding 1). The functions now reject any request that doesn't carry the service-role key as a Bearer token — closing a hole where anyone with the public anon key could POST a forged payload to a function URL and blast every subscriber with a fake/spoofed push. **No webhook changes needed** — the existing webhooks already send `Authorization: Bearer <SERVICE_ROLE_KEY>`, which is exactly what the check now requires. (1) Redeploy each: `npx supabase functions deploy notify-raid`, `… notify-dm`, `… notify-raid-message`, `… notify-raid-join`. (2) Confirm `verify_jwt` is **ON** for all four (Edge Functions → each → settings) as defense-in-depth (there's no `supabase/config.toml` in the repo, so the setting lives only in the dashboard). (3) Spot-check that real notifications still arrive after redeploy (post a raid / send a DM from a second account) — the webhooks should pass the new check unchanged. Optional negative check: a `curl` POST to a function URL with the anon key (or no auth) should now return `401`.
