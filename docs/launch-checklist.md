...
- [ ] **Set `GITHUB_BUG_REPORT_TOKEN` for in-app bug reports** (added 2026-06-10, `slice/bug-report`). The "Rapportér en fejl" form no-ops without the token — `/api/bug-report` answers 503 and users see the generic error. (1) Create a **fine-grained** GitHub PAT (GitHub → Settings → Developer settings → Fine-grained tokens) scoped to **only** the `digirenraz/pogosundet` repo with **only "Issues: Read and write"** permission. (2) Add it as `GITHUB_BUG_REPORT_TOKEN` in Vercel env vars (Production + Preview — it's a server-only secret, no `NEXT_PUBLIC_` prefix). (3) Spot-check on prod: hamburger menu → "Rapportér en fejl" → submit a test report → confirm a new issue appears in the repo with the `user-report` label and "Rapporteret af **\<trainer name\>** via appen." at the bottom, then close the test issue.
