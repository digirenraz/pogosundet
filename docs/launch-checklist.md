...
- [ ] **Spot-check hide-friend-code on prod** (added 2026-06-12, issue #101; **migration 020 applied 2026-06-12, PR #141 merged**). The column + code shipped; this is the cross-account check that can't be done from a dev machine: account A enables "Skjul min vennekode" under Rediger profil → from account B, A's card/detail shows "Ønsker ikke nye venner lige nu" with no copyable code; A still sees its own QR via /profile → "Vis QR kode". (Known residual, not blocking: the code is still readable via the raw Supabase API — see the decisions-log entry; column-level lockdown is a separate follow-up.)
...
- [ ] **Re-check notifications arrive (after reverting the Edge-function caller-auth check)** (updated 2026-06-12, `fix/restore-notifications`; security review Finding 1 **reopened**). The caller-auth check shipped earlier today **broke push delivery** (it 401'd the real webhook calls) and was **reverted + redeployed** — confirm on a device that a real push arrives again (post a raid / send a DM from a second account). **Finding 1 redo (future):** re-harden via a **custom shared-secret header** the webhook sends and the function verifies (e.g. `x-webhook-secret`), set as a function secret — and **send a real webhook through it before trusting it** (the previous attempt's "negative test" only hit the platform `verify_jwt` gate, never a genuine webhook call). Also confirm `verify_jwt` is ON for the four functions as baseline (no `supabase/config.toml` in the repo → dashboard-only).
