...
- [ ] **Spot-check push still arrives after the Edge-function caller-auth fix** (added 2026-06-12, `fix/edge-function-caller-auth`; security review Finding 1). **All four functions redeployed 2026-06-12 and the negative check passed** — a `curl` POST to a function URL with no auth / a bogus bearer now returns `401` (forged pushes blocked). Remaining: confirm a *real* notification still arrives (post a raid / send a DM from a second account) — the webhooks send `Authorization: Bearer <SERVICE_ROLE_KEY>`, which passes the new check, but this positive path is device-only. Also confirm `verify_jwt` is **ON** for all four (Edge Functions → each → settings) as defense-in-depth (no `supabase/config.toml` in the repo, so the setting lives only in the dashboard).
