// Client-side helpers for raid chat emoji reactions. Uses the browser Supabase
// client — never import this in Server Components.
//
// Mirror of `src/lib/chat/reactions-helpers.ts` but targets
// `raid_message_reactions`. Kept as its own file so raid helpers stay
// self-contained.
import { createClient } from '@/lib/supabase/client';

// Raw row from a Realtime INSERT/DELETE event on raid_message_reactions.
...
// Toggle a single emoji reaction on a raid chat message. Optimistic strategy:
// attempt INSERT first; if it fails with a PK conflict (23505) the row already
// exists, so flip to DELETE. Returns whether the net effect was an "add" or a
// "remove".
