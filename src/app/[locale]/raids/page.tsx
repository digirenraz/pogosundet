...
import { getActiveRaids } from '@/lib/raids/server-helpers';
import { RaidList } from '@/components/RaidList';
import { BottomNav } from '@/components/BottomNav';

// Raids page — shows active raids and a button to post a new one.
export default async function RaidsPage() {
  ...
  const { data: raids } = await getActiveRaids();
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-[76px] pb-[80px] px-4 flex flex-col gap-4">
        {raids.length === 0 ? (
          ...
          <RaidList raids={raids} currentUserId={user.id} />
        )}
      </main>
