...
Migrations `001`–`025` exist in `supabase/migrations/` (filenames are self-describing — run `ls supabase/migrations/` for the full list). **Applied through `024`; `025` NOT yet applied** (see its row below). Status of the recent ones:
...
| `025_fix_account_deletion_cascade` | ⬜ NOT applied | **apply before merge — to BOTH prod and preview** | Adds `ON DELETE CASCADE` to five `*_profile_fk` constraints (`raid_attendees`, `raid_messages`, `channel_messages`, `direct_messages` ×2) that were missing it since 002/003/008/014 — see the 2026-08-30 decisions log entry for the full root cause. Not query-referenced by new code (no new columns/tables), so it won't break prod on deploy either way — but the new `e2e/account-deletion.spec.ts` regression test needs it applied to **preview** to pass in CI, and the whole point is fixing a live bug, so apply to **prod** too before/alongside merging rather than after |
...
- **~~Investigate account deletion issues~~ — root cause found and fix built 2026-08-30, NOT yet applied/merged.** See the decisions log entry for the full writeup. Blocking step: apply migration `025_fix_account_deletion_cascade` to prod + preview (SQL editor, manual — see the migrations table), then verify `e2e/account-deletion.spec.ts` passes in CI before merging `slice/fix-account-deletion-cascade`.
...
|------------|-------------------------------------------------------|--------------------------------------|
| 2026-08-30 | **Account-deletion bug root-caused and fixed** (`slice/fix-account-deletion-cascade`, migration 025 — NOT yet applied to prod/preview as of this entry). The user-reported deletion problem from 2026-05-22 (never reproduced since) turned out to be a real schema bug, not a fluke: five `*_profile_fk` constraints — `raid_attendees` (002), `raid_messages` (003), `channel_messages` (008), and both `direct_messages` sender/recipient FKs (014) — were added purely so PostgREST could embed `profiles(trainer_name)` in queries, and were created **without `ON DELETE CASCADE`** (defaulting to `NO ACTION`), even though those same tables also carry a separate FK on the same column straight to `auth.users(id) ON DELETE CASCADE`. Deleting an account cascades to `profiles` and to those tables via two *independent* paths off the same `auth.users` row; Postgres doesn't guarantee which completes first, so if `profiles` gets cascaded away before the sibling table's own cascade clears out that user's rows, the `NO ACTION` constraint throws a foreign key violation and `auth.admin.deleteUser()` fails. This only reproduces for a user with real activity (a posted raid, a joined raid, a channel message, or a DM) — an empty throwaway account deletes cleanly, which is exactly why it went unnoticed: the only existing test (`server-helpers.test.ts`) mocks the Supabase call entirely, so it never exercises real FK behavior. Migration 024 (`message_reports`) already got this right with explicit `ON DELETE CASCADE` — the four older migrations didn't. Fix: migration `025_fix_account_deletion_cascade` (drop + re-add all five constraints with `ON DELETE CASCADE`); `route.ts` now `console.error`s the real Supabase error before returning the generic 500, so a future failure would actually show up in Sentry instead of being invisible; new `e2e/account-deletion.spec.ts` signs up a throwaway account, posts a channel message, then deletes it end-to-end — closing the exact test gap that let this slip through. **Not yet applied or merged** — needs the migration run against both prod and preview's SQL editors before the new e2e test can pass in CI (preview) and before real users stop hitting the bug (prod). Diagnosed entirely from the migration files as source of truth (the Supabase MCP connection was intermittently unauthorized this session) — recommend a live verification pass once reconnected. | The user asked to dig into a 3-month-old unreproduced bug report ahead of inviting more real users in — account deletion is a non-negotiable GDPR requirement, and the risk of a broken deletion flow grows with every new user. Root-causing from the migration files rather than guessing at symptoms found a genuine, previously-invisible correctness bug (not a one-off glitch), which is why it's being fixed at the schema level rather than papered over in application code. |
| 2026-08-30 | **Custom domain `pogosundet.dk` wired up, plus a new Terms of Use page** (`slice/terms-of-use`). Domain registered at one.com. Added `pogosundet.dk` + `www.pogosundet.dk` to the Vercel project (`www` 308-redirects → apex); no code changes needed for the domain itself (no hardcoded `vercel.app` refs anywhere, OAuth redirect already builds from `window.location.origin`). Updated Supabase Auth (prod project) Site URL + Redirect URLs to `pogosundet.dk`, and added `pogosundet.dk` to Google Cloud Console's OAuth consent screen Authorized domains (cosmetic — the OAuth redirect URI itself points at Supabase's `*.supabase.co/auth/v1/callback`, unaffected by the app domain). **One snag along the way:** Vercel auto-issued a TLS cert for `www.pogosundet.dk` but never got to the apex, so `pogosundet.dk` briefly served the `www` cert (SNI mismatch) — showed as "not secure" then `ERR_TOO_MANY_REDIRECTS` bouncing between the two hosts. Fixed with `vercel certs issue pogosundet.dk` to force issuance (resolved in ~15s); worth knowing if a future domain add on this Vercel account shows the same symptom — don't just wait, force it. **Separately, prepping the Google consent screen surfaced that the app had no Terms of Use**, only a Privacy Policy. Added `/terms` (mirrors the Privacy page's structure/tone: numbered sections, plain Danish, `Terms` i18n namespace in both `da.json`/`en.json`) covering service description, account rules, conduct, user-content ownership, moderation/bans (cross-referencing the existing report system), termination, no-warranty, changes-to-terms, and Danish governing law. Folded into the existing signup consent checkbox (now "Jeg accepterer privatlivspolitikken og vilkårene for brug") and the Google-signup consent notice, rather than adding a second separate checkbox — kept the required-fields count the same. New `e2e/terms.spec.ts` covers the page rendering and both consent links; changelog entry added. | The domain switch was purely operational (real players are being invited in via the two Facebook groups, so a branded URL matters now). The ToS wasn't planned — it came up because Google's OAuth consent screen review is the kind of moment that surfaces "we don't have basic legal pages" gaps. Google doesn't actually require a ToS link for the `email`/`profile` scopes this app uses, so it wasn't blocking, but the PM chose to add one now rather than defer, and to make it load-bearing (an actual accepted consent) rather than just a passively-linked page — matching how the Privacy Policy already works. |
