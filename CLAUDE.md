...
- **Android home-screen badge shows only a generic "new" dot, not a count** — expected Badging API limitation on Android Chrome (`setAppBadge` count isn't rendered by most launchers; iOS/desktop show the real number). Likely document as a known platform limitation rather than fix; confirm then note in `docs/notifications.md` / launch checklist.
