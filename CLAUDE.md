...
Nothing currently in flight as of 2026-05-25. Next slice not yet picked.
...
- **BottomNav unread badge flickers on navigation** — the Chat badge count is correct and now persists (PR #55), but it briefly blinks/resets when moving between pages. Cause: `BottomNav` is rendered per-page so it remounts on every navigation; `useDMUnread`/`useChannelUnread` start from a zero/empty state and re-fetch the real count on mount, so the badge flashes the stale-zero value for the fetch duration. Fix likely means persisting the count across navigations — e.g. lift the unread state into a shared layout/context above the per-page `BottomNav`, or seed `useDMUnread` from a cached value instead of 0. Low priority / polish.
