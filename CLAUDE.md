...
**Android DM-push follow-ups (found 2026-05-26).** Context: iOS is fully verified (DM notification + counting icon badge). Android push works (raids + DMs arrive once notification permission is granted). Remaining Android issues:

- **At least one iOS→Android DM push was lost** — intermittent delivery. Check the `notify-dm` Edge Function Logs/Invocations for send failures (4xx/410/FCM errors), confirm the Android `push_subscriptions` row is current, and consider best-effort FCM delivery when the device is asleep/offline. Reproduce + read logs next session.
