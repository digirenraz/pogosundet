...
Migrations `001`–`025` exist in `supabase/migrations/` (filenames are self-describing — run `ls supabase/migrations/` for the full list). **Applied through `025`** (all migrations are applied — there is no pending one). Status of the recent ones:
...
| `025_fix_account_deletion_cascade` | ✅ 2026-08-30 | before PR #229 | Adds `ON DELETE CASCADE` to five `*_profile_fk` constraints (`raid_attendees`, `raid_messages`, `channel_messages`, `direct_messages` ×2) that were missing it since 002/003/008/014 — see the 2026-08-30 decisions log entry for the full root cause. Applied to both prod and `pogosundet-preview` by the PM before the PR opened; the fix is empirically verified by `e2e/account-deletion.spec.ts` (signs up a throwaway account, posts a channel message, then deletes it) passing in CI against preview |
