...
Edge Functions (`supabase/functions/`): `notify-raid`, `notify-dm`, `notify-raid-message`, `notify-raid-join` — **all four deployed with their DB webhooks wired** (`notify-raid-join` verified on prod 2026-06-09). `notify-report` (moderation alerts) is **written but NOT yet deployed** — it needs `supabase functions deploy notify-report` plus a DB webhook on `INSERT` into `public.message_reports` carrying the `x-webhook-secret` header. Until then reports still land in `/admin`, just without the push.
