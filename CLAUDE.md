...
| 2026-05-22 | Fix: `last_seen_at` still null — write was inside Realtime SUBSCRIBED callback (unreliable on mobile) and SW wasn't bumped so old JS was served | Two cascading issues: (1) the write was conditional on WebSocket reaching SUBSCRIBED, which silently fails on mobile PWAs; fixed by moving it to a plain `useEffect` with HTTP. (2) PR #41 didn't bump the SW so users got the old cached JS without the fix; resolved by bumping SW v6→v7 to force fresh JS on next visit. Added `.then()` error logging to confirm. Verified working. |
| 2026-05-22 | Slice 15: PoGo screenshot avatar upload. New Supabase Storage bucket `avatars`. `src/lib/profile/avatar-helpers.ts` uploads to `{userId}/avatar.png` (upsert). `AvatarUploadSheet` lets the user drag + pinch-to-zoom the screenshot inside a 280px circle before cropping. | Initial approach was auto-crop at fixed coordinates — failed because the PoGo trainer-code screen places the avatar at a different position on different devices. Replaced with a drag-to-position editor. Pinch-to-zoom added immediately after first test showed the image needed zooming. Canvas crop uses the current offset + scale. `maxWidth: 'none'` required on the img to prevent global CSS from capping width and causing vertical distortion during zoom. |
| 2026-05-22 | iOS OAuth debugging saga — multiple broken approaches before landing on the working solution | Root cause: `createOAuthClient` (plain `@supabase/supabase-js`, localStorage) was introduced to work around iOS ITP purging the PKCE verifier cookie during the OAuth redirect chain. But the server-side route handler reads from cookies, not localStorage — they never matched, breaking prod login once the session expired. Correct fix: keep `createClient()` (SSR, cookies) on the login page; keep the original simple route handler (`exchangeCodeForSession` + `NextResponse.redirect('/')`). The auth callback had gone through several broken variants (200 HTML page, client-side exchange, implicit flow) before being fully reverted to the original. iOS Safari OAuth on preview deploys remains broken due to ITP — acceptable, prod works. |
