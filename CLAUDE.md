...
Recent shipped work (see the Decisions log for detail): raid chat notifications + unread badges (#109, migration 017, SW v16), raid-join notifications (#111, `notify-raid-join`), desktop layouts for Raids/Chat/Profil (#99), a series of PWA/OAuth/raid-navigation fixes (#91, #96–#98, #100, #115, #117), the navigation/perceived-performance pass (#120), and the **gyms database** (#121, migration 018, closes issue #93). Nothing currently in flight as of 2026-06-10. Next slice not yet picked.
...
- **Nearby-gym suggestions when posting a raid** (PM-confirmed follow-up to the gyms database, 2026-06-10) — use browser geolocation to sort/suggest gyms from the `gyms` table by distance to the user. Requires the PM to have seeded coordinates (`docs/gyms-seeding.md`). GDPR surface: geolocation permission prompt + Privacy Policy mention; location must not be stored.
- **"Show in maps" on the raid card** (PM-confirmed follow-up to the gyms database, 2026-06-10) — link the raid's `gym_name` to its `gyms` row and open the coordinates in the user's maps app. Trivial once gyms are seeded; decide fallback for raids whose gym has no coordinates.
