...
- **Add Amplitude for product analytics** — wire up Amplitude for usage analysis. Should be added soon. ⚠️ Currently the GDPR section states "No third-party analytics or tracking" as an in-place guarantee — adding Amplitude breaks that, so this requires explicit consent handling, EU data residency, a Privacy Policy update (+ `lastUpdated` bump), and a cookie/consent review before shipping.
