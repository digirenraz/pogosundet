...
**Android DM-push follow-ups (found 2026-05-26, troubleshoot next session).** Context: iOS is fully verified (DM notification + counting icon badge). Android push works (raids + DMs arrive once notification permission is granted — an unaccepted-permission reinstall initially masked this). Remaining Android issues:

- **Notification tap doesn't deep-link to the conversation** — `notificationclick` in `public/sw.js` (lines ~170–181) calls `client.focus()` on an already-open PWA window **without navigating to `event.notification.data.url`**; it only honours the URL via `clients.openWindow(url)`, which is reached only when *no* window exists. So tapping a DM push while the app is open/backgrounded just refocuses the last screen (player overview) instead of `/chat/dm/<sender_id>`. This is almost certainly why it "worked" on iOS (app fully closed → `openWindow` path) but not Android (app open → focus-only path). Affects raid notifications too. Fix: navigate the existing client to `url` (`client.navigate(url)` then `focus()`, with a fallback) and only `openWindow(url)` when there's no client. Verify on a real installed PWA.
